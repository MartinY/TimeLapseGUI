...
def startTimeLapse():
    ...
    lastPath.insert(0, (finalPath.split(PATH)[1]))

    logging.info('Start TimeLapse')

    currentDateTime = datetime.now()
    logging.info("Start date:\t{}".format(currentDateTime))
    startDateTime = currentDateTime + timedelta(seconds = 10)
    startDateTime -= timedelta(seconds = startDateTime.second % 10, microseconds = startDateTime.microsecond)
    ...
    while datetime.now() <= endDateTime:
        ...
        lastPic.delete(0, END)
        lastPic.insert(0, lastPicFile)
...
Label(mainWindow, text = 'Duration:').grid(row = 0, column = 0, sticky = W)
duration = Entry(mainWindow, width = 5)
duration.insert(0, '1')
duration.grid(row = 0, column = 1, sticky = W)
...
durationCombo.grid(row = 0, column = 1, sticky = E)
durationCombo.current(0)

Label(mainWindow, text = 'Period(s):').grid(row = 1, column = 0, sticky = W)
period = Entry(mainWindow, width = 5)
period.insert(0, '15')
period.grid(row = 1, column = 1, sticky = W)
period.config(validate = 'key', validatecommand = (validateNumberReg, '%P'))

Button(mainWindow, text = 'Start', command = lambda: threading.Thread(target = startTimeLapse).start(), width = 15).grid(row = 2, column = 0)
Button(mainWindow, text = 'Stop', command = lambda: threading.Thread(target = stopTimeLapse).start(), width = 15).grid(row = 2, column = 1)

Label(mainWindow, text = 'TimeLapse Path:').grid(row = 3, column = 0, sticky = W)
lastPath = Entry(mainWindow, width = 17)
lastPath.grid(row = 3, column = 1, sticky = W)
Label(mainWindow, text = 'Last Pic:').grid(row = 4, column = 0, sticky = W)
lastPic = Entry(mainWindow, width = 17)
lastPic.grid(row = 4, column = 1, sticky = W)

Button(mainWindow, text = 'Exit', command = exitTimeLapse, width = 15).grid(row = 5, column = 0, columnspan = 2)
